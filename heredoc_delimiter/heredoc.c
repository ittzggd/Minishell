...
/*   Updated: 2022/07/13 15:00:22 by hejang           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	preprocess_heredoc(void)
{
	...
	g_data.hd.heredoc_flag = TRUE;
	g_data.exit_status = 0;
	count = g_data.hd.heredoc_cnt;
	g_data.hd.heredoc_delimiter = ft_calloc(count + 1, sizeof(char *));
	if (!g_data.hd.heredoc_delimiter)
		ft_error("[preprocess_heredoc] heredoc_delimiter: allocation failed\n");
	g_data.hd.heredoc_fd = ft_calloc(count, sizeof(t_heredoc_fd *));
...
static void	heredoc_do_child(void)
{
	...
	while (idx < g_data.hd.heredoc_cnt)
	{
		if (g_data.lexer.ptype[i] == T_REDIRECTION)
			exec_heredoc(i, &idx);
		i++;
	}
	g_data.hd.heredoc_delimiter[idx] = NULL;
	g_data.hd.heredoc_flag = FALSE;
